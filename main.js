...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Listen for admin changes in another tab
  window.addEventListener('storage', function () {
    profile = JSON.parse(localStorage.getItem('sa_profile')) || { name:'', bio:'', image:'', followers:0 };
    books   = JSON.parse(localStorage.getItem('sa_books'))   || [];
    renderProfile();
    renderBooks(books);
  });
});
...
function renderProfile() {
  ...
  document.getElementById('profBio').textContent         = profile.bio  || 'Admin Panel থেকে লেখকের পরিচয় যোগ করুন।';
...
function renderBooks(list) {
  ...
  if (!list.length) {
    grid.innerHTML = '<div class="empty-state"><i class="fas fa-book-open"></i><p>এখনো কোনো বই যোগ করা হয়নি।</p><small>Admin Panel থেকে বই যোগ করুন।</small></div>';
    return;
  }
